...
{
    [Serializable]
    public class Implementation1 : IOrderManager, IDataManager
    {
        //Implementacija pirmam interfeisam
        private List<Order> pasutijums = new List<Order>();  //veidojam tukšu sarakstu ar pasutijumiem
        private List<Product> produkti = new List<Product>();   //veidojam tukšu sarakstu ar produktiem
        private List<Person> personas = new List<Person>();   //veidojam tukšu sarakstu ar personam
        public void AddOrder(Order f)
        {
            pasutijums.Add(f);// pievienoja pasūtījumu klāt, sarakstā
        }
        public void AddProduct(Product p)
        {
            produkti.Add(p);// pievienoja produktu klāt, sarakstā
        }
        public void AddProduct(string name, decimal price, int Amount)
        {
            OrderDetail Details = new OrderDetail();
            Product produkts = new Product();  //veidojam jauno objektu
            produkts.Name = name;
            produkts.Price = price;
            Details.product = produkts;
            Details.amount = Amount;
            produkti.Add(produkts); // pievienoja produktu klāt, sarakstā
        }
        public void DeleteCustomer(Customer1.Customer cust)
        {
            var allPersons = GetPersons();
            foreach (var persona in allPersons)
            {
                if (persona is Customer1.Customer)
                {
                    if (persona.Equals(cust))
                    {
                        personas.Remove(cust);
                        break;
                    }
                }
            }
        }  
        public void DeleteEmployee(Employee1.Employee empl)
        {
            var allPersons = GetPersons();
            foreach (var persona in allPersons)
            {
                if (persona is Employee1.Employee)
                {
                    if (persona.Equals(empl))
                    {
                        personas.Remove(empl);
                        break;
                    }
                }
            }
        }
        public void DeleteProduct(Product1.Product product)
        {
        ...
        {
            personas.Add(persona); // pievienoja personu klāt, sarakstā (gan darbiniekus, gad pasūtītājus)
        }
        ...
        public void CreateTestData()
        {
            ...
            //pievienojam visu sarakstos , visu PĒC KĀRTA, KĀ TIKA NOKODĒTS AUGŠĀ
            AddOrder(pasutijums);
            AddOrder(pasutijums2);
            AddOrder(pasutijums3);
            AddPerson(customer1);
            AddPerson(customer2);
            AddPerson(customer3);
            AddPerson(respEmpl1);
            AddPerson(respEmpl2);
            AddPerson(respEmpl3);
            AddProduct(produkts1_1);
            AddProduct(produkts1_2);
            AddProduct(produkts1_3);
            AddProduct("Asus ZenBook 14", 849M, 5);  
            AddProduct("Dell Vostro 14 ", 546.34M, 3);
            AddProduct(produkts2_1);
            AddProduct(produkts2_2);
            AddProduct("Dell Latitude 13", 1579.99M, 2);
            AddProduct(produkts3_1);
            AddProduct("HP ZBook 15v", 1855.99M, 7);
        }
        public void Save(string FileName)
        {
             Stream TestFileStream = File.Create(FileName);
            BinaryFormatter serializer = new BinaryFormatter();
            serializer.Serialize(TestFileStream, pasutijums);
            serializer.Serialize(TestFileStream, produkti);
            serializer.Serialize(TestFileStream, personas);
        ...
        public void Load(string FileName)
        {
            if (File.Exists(FileName))
            {
                ...
                var pas1 = (List<Order>)deserializer.Deserialize(TestFileStream);
                var pas2 = (List<Product>)deserializer.Deserialize(TestFileStream);
                var pas3 = (List<Person>)deserializer.Deserialize(TestFileStream);
                pasutijums = pas1;
                produkti = pas2;
                personas = pas3;
                TestFileStream.Close();
        ...
        public string Print()
        {
            string s = "";
            s += "      ++++++++++++++++\n";
            s += "  Pasutijumu kolekcijas saturs (Orders List): \n";
            s += "      +++++++++++++++++\n";
            s += PrintOrders();
            s += "\n       ++++++++++++++++\n";
            s += "  Produktu kolekcijas saturs (Products List) :\n";
            s += "       ++++++++++++++++\n";
            s += PrintProducts();
            s += "\n       ++++++++++++++++\n";
            s += "  Personu kolekcijas saturs (Persons List) :\n";
            s += "       ++++++++++++++++\n";
            s += PrintPeople();
            return s;
        }
        public string PrintProducts()
        {
            string s = "";
            var allProducts = GetProducts();
            foreach (var produkts in allProducts)
            {
                s += produkts.Name.ToString() + ", " + produkts.Price.ToString() + " EUR \n";
            }
            return s;
        }
        public string PrintOrders()
        {
            ...
            int flag = 0;
            foreach (var pasutijums in allOrders)
            {
                flag++;
                s += "\n********" + flag + ". pasutijums********\n\n";
                s += "       Info par pasutijumu:\n" + pasutijums.ToString();
        ...
        public string PrintPeople()
        {
            ...
            {
                s += person.FullName.ToString() + ", EMail: " + person.EMail.ToString() + "\n";
            }
            return s;
        }
        public string PrintPasutitajus()
        {
            string s = "";
            var allPersons = GetPersons();
            foreach (var i in allPersons)
            {
                if (i is Customer)
                {
                    s += i.FullName + ", Epasts: " + i.EMail + "\n";
                }
            }
            return s;
        }
        public string PrintDarbiniekus()
        {
            string s = "";
            var allPersons = GetPersons();
            foreach (var i in allPersons)
        ...
        public void Reset()
            {
                pasutijums = new List<Order>();
                personas = new List<Person>();
                produkti = new List<Product>();
            }
    }
}
